import { ObjectsMap, arrayFrom, arrayPull, objectPull } from '@noeldemartin/utils/helpers/index';
...
export function map<T extends object, S extends keyof T>(
    value: Iterable<T> = [],
    key?: S | ((item: T) => string),
): ObjectsMap<T> {
    if (!Array.isArray(value) || typeof value[0] !== 'string') return ObjectsMap.createFromArray(value, key as S);
...
export function pull(obj: unknown, key: unknown): unknown {
    return Array.isArray(obj) ? arrayPull(obj, key as number) : objectPull(obj as Obj, key as keyof Obj);
}
